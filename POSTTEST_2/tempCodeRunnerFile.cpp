...
int aturPosisi(Kereta* arr, int kiri, int kanan) {
    string pivot = (*(arr + kanan)).nama;
    int i = kiri - 1;
    ...
    swapKereta(arr + i + 1, arr + kanan);
    return i + 1;
}

void quickSortNama(Kereta* arr, int indeksAwal, int indeksAkhir) {
    if (indeksAwal < indeksAkhir) {
        int pi = aturPosisi(arr, indeksAwal, indeksAkhir);
        quickSortNama(arr, indeksAwal, pi - 1);
        quickSortNama(arr, pi + 1, indeksAkhir);
    }
}

int boyerCari(string teks, string pola) {
    int m = pola.length();
    int n = teks.length();
    int dataChar[256];
    for (int i = 0; i < 256; i++) dataChar[i] = -1;
    for (int i = 0; i < m; i++) {
        dataChar[(int)pola[i]] = i;
    }
    int geser = 0;
    while (geser <= (n - m)) {
        int j = m - 1;
        ...
        if (j < 0) {
            ...
            int lompat = j - dataChar[(int)teks[geser + j]];
            geser += (lompat > 1 ? lompat : 1);
        }
    }
    return -1;
}

void cariNamaBoyer(Kereta* arr, int total) {
    string cariNama;
    cout << "\nCari nama kereta: ";
    cin.ignore();
...
int main() {
    ...
    do {
        ...
        if (pilih == 1) {
            ...
            quickSortNama(dataKereta, 0, jumlahKereta - 1);
            lihat(dataKereta, jumlahKereta);
            lanjutsekalianbersih();
        }
        else if (pilih == 8) {
            cariNamaBoyer(dataKereta, jumlahKereta);
            lanjutsekalianbersih();
        }
    } while (pilih != 9);
    cout<<"\nKeluar Program";
